    ...
    Subscription.where({ active: true }, function(err, subscriptions){
      ...
      Weebly.call(function(err, items){
        ...
        var email = new sails.config.sendgrid.Email({
          to: subscriptions.map(function(subscription){
        ...
        email.setHtml(items[0].description + '<br /><br /><p>To unsubscribe <a href="http://' + BaseUrl(req) + '/unsubscribe/-email-/-customerKey-">click here</a>');
